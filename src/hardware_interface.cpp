...
void cmd_vel_callback(const geometry_msgs::Twist &cmd_vel_in)
{
    ...
    double x = cmd_vel_in.linear.x/10; //sementara karena max_vel mpc nya 2 m/s

    int steer_angle = base_angle + int(rot*rad2deg) ;
    int speed = base_speed - int(x*666.66667); // konversi dari m/s ke bentuk PWM microsecond
    
    cmd_vel_out.linear.x = speed;
    cmd_vel_out.angular.z = steer_angle;
...
int main(int argc, char **argv)
{
    ...
    pub_cmd_vel = n.advertise<geometry_msgs::Twist>("/car/cmd_vel", 100);
